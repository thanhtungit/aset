...
add_action('init','init_onepay');
 
function init_onepay(){
    function woocommerce_add_onepay_gateway( $methods ) {
			$methods[] = 'OnePay_Domestic';
			return $methods;
	   }
 ...
 *
 */
class OnePay_Domestic extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->order_button_text  = __( 'Proceed to OnePay', 'onepay' );
		$this->method_title       = __( 'OnePay Domestic Gateway', 'onepay' );
		$this->method_description = __( 'OnePay supports all major bank ATMs in Vietnam.', 'onepay' );
		$this->supports           = array(
			'products',
		);
		...
		$this->description   = $this->get_option( 'description' ) . sprintf( '<br/><div align="center"><img src="%s"></div>', TEMPLATE_PATH . '/images/onepay_domestic.png' );
	...
	public function init_form_fields() {
		$this->form_fields = include( 'onepay/onepay-settings.php' );
	}
